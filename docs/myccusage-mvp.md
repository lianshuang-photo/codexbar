...
- Show the installed collector version when available.
  If the binary is missing, CodexBar shows `npm install -g ccusage-cherry-collector@latest`.
...
- `MyCCusageSyncRunner` locates and runs `ccusage-cherry-collector sync`.
  It also reads `ccusage-cherry-collector --version` for status display.
